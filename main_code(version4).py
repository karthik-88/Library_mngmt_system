...
def lend():
    while True:
        ...
        if not available_books:
            print("No books are currently available for lending.")
            break  # Exit if no books are available

        # Display available books
        for book in available_books:
            print(book[0])

        book_selected = input('Enter the name of the book to lend: ').strip()
        
        # Check if the selected book exists
        command("SELECT Quantity_Available FROM books WHERE Book_Name = %s", (book_selected,))
        result = cursor.fetchone()

        if result and result[0] > 0:
            # Proceed with lending the book
            borrowers_name = input('Enter Borrower Name: ').strip()
            contact = int(input('Enter contact no.: '))
            
            # Insert borrower into the BORROWER table
            command("""
            ...
            command("""
                ...
                WHERE Book_Name = %s
            """, (book_selected,))
            print(f"Book '{book_selected}' lent successfully to {borrowers_name}.")

            # Ask if more records need to be added
            more_records = input('Do you want to lend another book (Y/N)? ').upper()
            if more_records != 'Y':
                break  # Exit if user doesn't want to lend more books
        else:
            print(f"Sorry, '{book_selected}' is not available right now or doesn't exist.")
